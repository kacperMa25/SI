...
def h1(board, hFun):
    if board is None:
        return 0
    getter = getattr(board, hFun, None)
    if callable(getter):
        return getter()
    return 0 if (not getattr(board, "conflicts", lambda: False)()) else float("inf")
...
class BoardPerm:
    ...
    def count_attacks_queens(self):
        attacks = self.count_attacks()
        placed = self._size
        if self._perm:
            placed = len(self._perm)
        missing = max(0, getattr(self, "_target_size", 0) - placed)
        return attacks + missing
    ...
    @staticmethod
    def BestFS(size, hFun="getAttacksAndQueens"):
        start = BoardPerm()
        start._target_size = size
        heap = []
        counter = 0
        h0 = h1(start, hFun)
        ...
        while heap:
            ...
            for t in ts:
                ...
                if key_t in closed:
                    continue
                if t._size == size and not t.conflicts():
                    openCount += 1
                    return t, openCount, closeCount
                if not t.conflicts():
                    t._target_size = size
                    counter += 1
                    heapq.heappush(heap, (h1(t, hFun), counter, t))
                    openCount += 1
...
class BoardMatrix:
    ...
    def count_attacks_queens(self):
        attacks = self.count_attacks()
        placed = sum(sum(row) for row in self._matrix)
        target = getattr(self, "_target_size", self._size)
        missing = max(0, target - placed)
        return attacks + missing
        ...
        while opened:
            ...
            if queenCount < size:
                ...
                for t in ts:
                    ...
                    if (
                        not t.conflictsOptimized()
                        and t._matrix not in opened
    ...
    @staticmethod
    def BestFS(size, hFun="countAttacks"):
        start = BoardMatrix()
        start.makeBoard([[0] * size for _ in range(size)])
        start._target_size = size
        ...
        while heap:
            h, _, s = heapq.heappop(heap)
            key_s = tuple(tuple(row) for row in s._matrix)
            ...
            queenCount = sum(sum(row) for row in s._matrix)
            if queenCount == size and not s.conflictsOptimized():
                return s, closeCount, openCount

            ts = s.generateChildren(s)
            for t in ts:
                key_t = tuple(tuple(row) for row in t._matrix)
                if key_t in closed:
                    continue
                qt = sum(sum(row) for row in t._matrix)
                if qt == size and not t.conflictsOptimized():
                    openCount += 1
                    return t, closeCount, openCount

                if not t.conflictsOptimized():
                    t._target_size = size
                    counter += 1
                    heapq.heappush(heap, (h1(t, hFun), counter, t))
                    openCount += 1

        return None, closeCount, openCount
    ...
    def generateChildren(parent):
        ...
        for row in range(size):
            for col in range(size):
                ...
                b._target_size = getattr(parent, "_target_size", parent._size)
                children.append(b)
        return children


if __name__ == "__main__":
    print("BoardMatrix BestFS with countAttacks heuristic:")
    sol, closed, opened = BoardMatrix.BestFS(6, hFun="countAttacks")
    if sol:
        sol.printBoard()
    print("closed:", closed, "opened:", opened)
    print()

    print("BoardMatrix BestFS with getAttacksAndQueens heuristic:")
    sol, closed, opened = BoardMatrix.BestFS(6, hFun="getAttacksAndQueens")
    if sol:
        sol.printBoard()
    print("closed:", closed, "opened:", opened)
    print()

    print("BoardPerm BestFS with getAttacks heuristic:")
    sol, opened, closed = BoardPerm.BestFS(6, hFun="getAttacks")
    if sol:
        sol.printPerm()
    print("opened:", opened, "closed:", closed)
    print()

    print("BoardPerm BestFS with getAttacksAndQueens heuristic:")
    sol, opened, closed = BoardPerm.BestFS(6, hFun="getAttacksAndQueens")
    if sol:
        sol.printPerm()
    print("opened:", opened, "closed:", closed)
